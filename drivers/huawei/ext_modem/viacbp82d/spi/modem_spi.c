...
static struct sdio_read_func read_func = {0};
#define SK_BUFFER_RESERVE	(12)
#define SPI_HEAD_SIZE	        (5)
#define SEND_BLOCK_SIZE     (512  - 4)
#define SPI_HEAD_MAX_INDEX	        (SPI_HEAD_SIZE - 1)
...
static void spi_tx_rx_printk(const void *buf, unsigned char type)
{
	...
	if(type == 1){
		hwlog_info("write %d to channel %d tx cnt %d/[chan %u tx total bytes:%d]>>",
				count, *(print_buf+1), spi_tx_cnt, print_channel, spi_tx_byte_count_on_channel[print_channel]);
	}
	else{
		hwlog_info("read %d from channel %d rx cnt %d/[chan %u rx total bytes:%d]<<",
				count, *(print_buf+1), spi_rx_cnt, print_channel, spi_rx_byte_count_on_channel[print_channel]);
	}
	...
	for(i = 0; i < count + 4; i++)
	{
		if ((SPI_AT_CHANNEL_NUM == spi_channel) \
			...
			printk("%x-", *(print_buf+i)); //must use printk to print raw data, which do not need TAG
		}
	}

	//channel is from 0 to 15, channel count = tty count + 1
	if (1 == type) {
		printk("; tx count of channel 0-15:[");
		for (i = 0; i < SPI_TTY_NR; i++) {
			printk("%x,", spi_tx_cnt_on_channel[i]); //must use printk to print raw data, which do not need TAG
		}
		printk("%x]\n", spi_tx_cnt_on_channel[SPI_TTY_NR]);
	} else {
		printk("; rx count of channel 0-15:[");
		for (i = 0; i < SPI_TTY_NR; i++) {
			printk("%x,", spi_rx_cnt_on_channel[i]);
		}
...
EXPORT_SYMBOL(spi_rx_last_packet_dump);
...
int modem_on_off_ctrl_chan(unsigned char on)
{
	...
	do {
		...
		if (0 == remain_time) {
			hwlog_err("%s: wait CBP software flow control disable timeout in 200ms.\n", __func__);
			wait_soft_flow_ctrl_times++;
			if (wait_soft_flow_ctrl_times > MAX_WAIT_SOFT_FLOW_CTRL_TIMES) {
				hwlog_err("%s: wait CBP software flow control disable has reached MAX WAIT TIMES.\n", __func__);
				return -1;
			}
		}
	} while (0 == remain_time);
...
static void spi_buffer_in_print(struct spi_modem_port *port, struct spi_buf_in_packet *packet)
{
	...
	hwlog_info("spi channel %d buffer in %d bytes data<<", (port->index+1), packet->size);
	count = packet->size;
	if( count > 20){
		count = 20;
	}
	for(i = 0; i < count; i++)
	{
		printk("%x-", *(packet->buffer+i)); //must use printk to print raw data, which do not need TAG
	}
	printk("\n");
}
...
static int modem_tty_install(struct tty_driver *driver, struct tty_struct *tty)
{
	...
	if (ret) {
		hwlog_err("%s %d: tty_init_termios return error, ret=%d.\n", __func__, __LINE__, ret);
		return ret;
	}
...
static void respond_cflag80_packet_work(struct work_struct *work)
{
	 ...
	 */
		...
		do {
			status = spi_sync(port->modem->spi, &m);
			if (status) {
	...
	if (ret) {
		if (-1 == ret) {
			err_flag = 1;
			hwlog_err("%s: respond_cflag80_packet_work has wait data ack 8s.\n", __func__);
		} else if (-2 == ret) {
			hwlog_err("%s: VIA reset happened, just return.\n", __func__);
		}
		goto down_sem_fail;
	}
	...
	if(modem->cbp_data->flow_ctrl_enable){
		if(oem_gpio_get_value(modem->cbp_data->cbp_flow_ctrl->wait_gpio)!=modem->cbp_data->gpio_flow_ctrl_polar){
			...
			while(1){/*print added for testing,to be removed*/
				...
				hwlog_info("spi_write_port_work wait flow_ctrl begin, current flow_ctrl gpio level is %d\n", \
				...
				if (ret_wait_hardware_flow_ctrl) {
					if (-1 == ret_wait_hardware_flow_ctrl) {
						...
						hwlog_err("%s %d: has wait hardware flow ctrl disable for 8s.\n", __func__, __LINE__);
					} else if (-2 == ret_wait_hardware_flow_ctrl) {
						hwlog_err("%s %d: VIA reset has happened, just return.\n", __func__, __LINE__);
					}
					goto down_out;
				}
				hwlog_info("spi_write_port_work wait flow_ctrl end, current flow_ctrl gpio level is %d\n", \
						oem_gpio_get_value(modem->cbp_data->cbp_flow_ctrl->wait_gpio));
 ...
 */
	 	if (TRANS_STATUS_WAITING_INIT_SEGMENT == modem->spi_tx_state){
			...
			*(modem->trans_buffer + 3) = 0xFF & frmlen;
			...
			if (write_len < 24) {
				memcpy(spi_tx_last_packet_dump, modem->trans_buffer, write_len);
			} else {
				memcpy(spi_tx_last_packet_dump, modem->trans_buffer, 24);
			}
		...
		if (ret_wait_ack) {
			if (-1 == ret_wait_ack) {
				err_flag = 1;
				hwlog_err("%s: spi_write_port_work has wait data ack for 8s.\n", __func__);
...
static void rnic_read_packet(unsigned char *buffer, unsigned int datalen)
{
    ...
    if (datalen > 1) {
        ...
        if (pdnId == CDMA_PDNID) {
            pdnId = read_func.ucPDNId;
        }
        hwlog_info("RNIC__ %s pdnId = %d\n",__func__,pdnId);
    }
    else {
    ...
    if (read_func.cbp_read_buff != NULL) {
        ...
        if (skb != NULL) {
            ...
            ret = read_func.cbp_read_buff(pdnId, skb);
            if (RNIC_OK == ret) {
                hwlog_info("RNIC__ %s read OK\n",__func__);
            }
            else {
...
{
    read_func.cbp_read_buff = pFunc;
    read_func.ucPDNId = ucPDNId;
    return RNIC_OK;
}
...
unsigned long SDIO_UL_SendPacket(struct sk_buff *pstData, unsigned char ucPDNId)
{
    ...
    hwlog_info("RNIC__ %s: enter, SDIO_UL_SendPacket\n", __func__);
...
unsigned long rnic_send_packet(struct sk_buff *pstData, unsigned char ucPDNId)
{
    ...
    if(down_interruptible(&port->modem->spi_sem)){
        hwlog_err("RNIC__ %s %d down_interruptible failed.\n", __func__,__LINE__);
        ret =  RNIC_ERROR;
        goto down_err;
    }
...
static void spi_read_port_work(struct work_struct *work)
{
	...
	do{
		...
		do {
			...
			if (status) {
				hwlog_err("%s - sync error: status=%d, spi_failed_times=%u.\n", __func__, status, spi_failed_times);
				spi_failed_times++;
				if(spi_failed_times > SPI_TRANS_MAX_TRY_TIMES) {
					hwlog_err("%s - sync error has reached MAX FAILed times.\n", __func__);
					return;
				} else {
					if(-ESHUTDOWN == status) {
						msleep(2000);
		...
		if (TRANS_STATUS_WAITING_INIT_SEGMENT == modem->spi_rx_state) {
			...
			do {
				status = spi_sync(modem->spi, &m);
				if (status) {
					hwlog_err("%s - spi read first segment remain data error=%d, spi_failed_times=%u.\n", __func__, status, spi_failed_times);
					spi_failed_times++;
					if (spi_failed_times > SPI_TRANS_MAX_TRY_TIMES) {
						hwlog_err("%s - sync error has reached MAX FAILed times.\n", __func__);
						return;
		...
		if (ret_read_wait_ack) {
			if (-1 == ret_read_wait_ack) {
				err_flag = 1;
				hwlog_err("%s: spi_read_port_work has wait data ack for 8s.\n", __func__);
			} else if (-2 == ret_read_wait_ack) {
				hwlog_err("%s: VIA reset has happened.\n", __func__);
			}
			//error wait data ack, just continue first parse already read data, then notify CBP to produce ramdump.
		}
	...
	if (total_len < 24) {
		memcpy(spi_rx_last_packet_dump, modem->msg, total_len);
	} else {
		memcpy(spi_rx_last_packet_dump, modem->msg, 24);
	}
	  ...
	  if(TRANS_STATUS_FINISHED == modem->spi_rx_state) {
		...
		if (ret < 0)
		{
			...
			goto out;
              ...
              if (index == SPI_TTY_NR - 1 && read_func.cbp_read_buff != NULL) {
                  mutex_lock(&port->spi_buf_in_mutex);
		...
		void *buf, size_t len)
{
	...
	do {
		...
		if (status) {
			hwlog_err("%s - sync error: status=%d, spi_write_failed_times=%u.\n", __func__, status, spi_write_failed_times);
			spi_write_failed_times++;
			if (spi_write_failed_times > SPI_TRANS_MAX_TRY_TIMES) {
				hwlog_err("%s - sync error has reached MAX FAILed times.\n", __func__);
				return;
			} else {
				if (-ESHUTDOWN == status) {
...
int  modem_spi_init(struct cbp_platform_data *pdata)
{
	...
	hwlog_info(" %s: spi driver is initialized successfully!\n",__func__);
	return ret;
exit_tty:
