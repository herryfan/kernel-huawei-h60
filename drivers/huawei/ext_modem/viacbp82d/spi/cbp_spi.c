...
#define MAX_WAIT_DATA_ACK_TIMES    (3)
#define MAX_WAIT_HARDWARE_FLOW_CTRL_TIMES    (3)
...
static int data_ack_wait_event(struct cbp_wait_event *pdata_ack)
{
	...
	do {
		remain_time = wait_event_timeout(cbp_data_ack->wait_q, ((MODEM_ST_READY == atomic_read(&cbp_data_ack->state))||
			(cbp_power_state==0)), msecs_to_jiffies(2000));
		if (0 == cbp_power_state) {
			hwlog_err("%s: VIA CBP reset happened!.\n", __func__);
			return -2;
		} else if(0 == remain_time) {
			hwlog_err("%s: wait CBP response data ack timeout in 2s.\n", __func__);
			failed_times++;
			if (failed_times > MAX_WAIT_DATA_ACK_TIMES) {
				hwlog_err("%s: wait CBP response data ack has reached MAX WAIT TIMES.\n", __func__);
				return -1;
			}
		}
	} while (0 == remain_time);
...
static int flow_ctrl_wait_event(struct cbp_wait_event *pflow_ctrl)
{
	...
	do {
		...
		if (0 == cbp_power_state) {
			...
			hwlog_err("%s: wait CBP hardware flow control disable timeout in 2s.\n", __func__);
			wait_hardware_flow_ctrl_times++;
			if (wait_hardware_flow_ctrl_times > MAX_WAIT_HARDWARE_FLOW_CTRL_TIMES) {
				hwlog_err("%s: wait CBP hardware flow control disable has reached MAX WAIT TIMES\n", __func__);
				return -1;
			}
		}
	} while (0 == remain_time);
