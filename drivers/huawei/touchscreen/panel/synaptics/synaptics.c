...
#define HOLSTER_SWITCH_OFFSET      10
...
static int synaptics_set_holster_switch(u8 holster_switch)
{
	...
	holster_enable_addr = rmi4_data->rmi4_feature.f51_ctrl_base_addr + HOLSTER_SWITCH_OFFSET;
	TS_LOG_INFO("synaptics_set_holster_switch value is %d  set_holster_addr : 0x%2x\n",holster_enable_addr,holster_enable_addr);
...
static int synaptics_set_palm_switch(u8 palm_switch)
{
	...
	palm_enable_offset = rmi4_data->rmi4_feature.geswakeup_feature.f12_2d_ctrl22_palm;
	palm_enable_addr = rmi4_data->rmi4_feature.f12_ctrl_base_addr + palm_enable_offset;
	TS_LOG_INFO("set palm addr : 0x%2x\n",palm_enable_addr);
...
static int synaptics_palm_switch(struct ts_palm_info *info)
{
	...
	switch (info->op_action) {
		case TS_ACTION_READ:
			...
			if (retval < 0) {
				TS_LOG_ERR("get glove switch(%d), failed : %d", info->palm_switch, retval);
			}
		...
		case TS_ACTION_WRITE:
			...
			if (retval < 0) {
				TS_LOG_ERR("set palm switch(%d), failed : %d", buf, retval);
			}
			break;
		default:
			TS_LOG_ERR("invalid switch status: %d", info->palm_switch);
			retval = -EINVAL;
			break;
	}
...
#endif
	retval = synaptics_set_palm_switch(rmi4_data->rmi4_feature.geswakeup_feature.palm_switch);//rmi4_data->synaptics_chip_data->rs_info.gesture_palm_set_flag
	if (retval < 0) {
		...
		unsigned char page)
{
	...
	TS_LOG_DEBUG("f51 init , ctrl_base_addr = %d, page = %d\n", fd->ctrl_base_addr, page);
	return 0;
}
	...
	if (!list_empty(&rmi->support_fn_list)) {
		list_for_each_entry(fhandler, &rmi->support_fn_list, link) {
			if (fhandler->fn_number == SYNAPTICS_RMI4_F12)
				f12_ctrl_base = fhandler->full_addr.ctrl_base;
			}
	}
